...
module.exports.run = async function({ api, event,Threads }) {
  ...
  if  (checkban.includes(checkban[0])) return
    ...
    api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "DrawDev ©️" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
    return api.sendMessage(`Kết nối thành công với bot của DrawDev ${global.config.PREFIX}help để xem toàn bộ lệnh của bot <3`, threadID);
  }
  else {
    try {
